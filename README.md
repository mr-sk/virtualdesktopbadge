# <img src="screenshots/icon.png" height="34" align="top"> Virtual Desktop Badge

[![CI](https://github.com/mr-sk/virtualdesktopbadge/actions/workflows/ci.yml/badge.svg)](https://github.com/mr-sk/virtualdesktopbadge/actions/workflows/ci.yml) [![Latest release](https://img.shields.io/github/v/release/mr-sk/virtualdesktopbadge)](https://github.com/mr-sk/virtualdesktopbadge/releases/latest) ![Platform](https://img.shields.io/badge/macOS-13%2B-blue) [![Downloads](https://img.shields.io/github/downloads/mr-sk/virtualdesktopbadge/total)](https://github.com/mr-sk/virtualdesktopbadge/releases)

A tiny OSX menu-bar app that shows **which desktop (Space) you're on** as a number in a small box. No Dock icon, no window,  just the number and some context aware text.

OSX has keyboard shortcuts to jump between desktops (ie: `ctrl+1`, `ctrl+2`, …) but gives no on-screen indication of which one you're currently on. Virtual Desktop Badge fills that gap.
...
The app is ad-hoc signed (not notarized), so OSX blocks it on first launch. Allow it once with either:
...
The `ctrl+N` instant path uses a global key monitor, which OSX gates behind **Accessibility** permission. On first launch Virtual Desktop Badge prompts for it:
...
- Click the badge for the menu:
  ...
  ![The Virtual Desktop Badge menu](screenshots/menu.png)

## How it works

OSX exposes **no public API** for "what Space number am I on," so Virtual Desktop Badge uses a hybrid of two signals:
...
The private API is **isolated to a single file** (`Sources/virtualdesktopbadge/CGSBridge.swift`). If a future OSX changes it, only that file needs attention, and the keyboard path keeps working regardless.
...
- Auto app-labels only reflect the desktop you're currently viewing; OSX doesn't expose the windows on desktops you aren't looking at (which is all the badge needs).
- The Space-number derivation relies on a private, undocumented OSX API; it may need updating on major macOS releases.
